...
function randomDelta(price: number) {
  // simulate small market-like fluctuation ±0.8%
  const pct = (Math.random() - 0.48) * 0.016;
  return Math.round(price * pct / 500) * 500; // round to nearest 500đ
}
...
export default function PriceTicker() {
  const [ticks, setTicks] = useState<Record<string, Tick>>(() => {
    ...
    BASE_PRODUCTS.forEach(p => {
      init[p.code] = { price: p.price, change: 0, changePct: 0, direction: 'flat', flash: false };
    });
    return init;
  });

  const prevPrices = useRef<Record<string, number>>(
    Object.fromEntries(BASE_PRODUCTS.map(p => [p.code, p.price]))
  );

  // tick every 1.8s, update 2–4 random products
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setTicks(prev => {
        const next = { ...prev };
        shuffled.forEach(p => {
          const delta = randomDelta(prevPrices.current[p.code]);
      ...
      setTimeout(() => {
        setTicks(prev => {
          const next = { ...prev };
          shuffled.forEach(p => {
            if (next[p.code]) next[p.code] = { ...next[p.code], flash: false };
          });
          return next;
        });
      }, 600);
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${!tick.flash ? 'bg-gray-900 border-gray-800' : ''}
                `}
              >
                {/* Code badge */}
                ...
                {/* Price */}
                <div className={`text-lg font-bold tabular-nums transition-colors duration-300 ${
                ...
                <div className={`flex items-center gap-1 text-xs font-semibold ${
                  tick.change > 0 ? 'text-green-400' : tick.change < 0 ? 'text-red-400' : 'text-gray-500'
                }`}>
                  {tick.change > 0 && (
                ...
                <div className="mt-3 h-1 rounded-full bg-gray-800 overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${
                      isUp ? 'bg-green-500' : isDown ? 'bg-red-500' : 'bg-gray-600'
                    }`}
                    style={{
                      width: `${Math.min(100, Math.max(10, 50 + tick.changePct * 20))}%`,
                    }}
                  />
                </div>
              </div>
