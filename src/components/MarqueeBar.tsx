...
// Separate pure display item — reads from a ref, no re-render on tick
function TickItem({ code, name, tickRef }: { code: string; name: string; tickRef: React.MutableRefObject<Record<string, Tick>> }) {
  const [tick, setTick] = useState<Tick>({ price: PRODUCTS.find(p => p.code === code)!.price, change: 0, pct: 0 });

  useEffect(() => {
    // subscribe to updates via custom event
...
export default function MarqueeBar() {
  ...
  // Price simulation — dispatches custom events, never touches animation DOM
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      picks.forEach(p => {
        ...
        const tick: Tick = {
          ...
          pct:    ((newPrice - p.price) / p.price) * 100,
        };
  ...
  // Duplicate list for seamless CSS loop (2 identical tracks)
  ...
  return (
    <div className="bg-gray-950 border-b border-gray-800 h-9 flex items-center overflow-hidden relative z-40">
      ...
      {/* Scrolling track — animation lives here, never re-renders */}
      <div className="overflow-hidden flex-1 relative">
        <div className="flex animate-marquee whitespace-nowrap">
          {track.map((p, i) => (
            <TickItem key={i} code={p.code} name={p.name} tickRef={tickRef} />
          ))}
        </div>
      </div>
